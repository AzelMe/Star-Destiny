...
Game_Screen.prototype.updateTilemapMargin = function() {
	const scale = Math.min(this._zoomScale, this._zoomScaleTarget);
	this.refreshTilemapMargin(scale);
	this._completeZoomIn = 0;
};

Game_Screen.prototype.refreshTilemapMargin = function(scale) {
	if(SceneManager._scene.constructor === Scene_Map) {
		const margin = Math.max(0, (1 / scale - 1) * Graphics.width / 2);
		...
		if (spriteset._parallax) {
			spriteset._parallax.move(
				-margin,
				-margin,
				Graphics.width + (margin * 2),
				Graphics.height + (margin * 2)
			);
		}
	}
};
